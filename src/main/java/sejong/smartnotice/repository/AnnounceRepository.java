...
public interface AnnounceRepository extends JpaRepository<Announce, Long> {
    ...
    List<Announce> findAllAnnounceToTownById(Long townId);

    @Query("select distinct a from Announce a join fetch a.atList at join fetch at.town where at.town in(?1)")
    List<Announce> findAllAnnounceToTown(List<Town> townList);
}
