...
public class TownDataService {
    ...
    // 캐시 데이터 생성 및 동기화
    public TownData initTownData(Long townId) {
        ...
        for (Object[] result : alertCntList) {
            ...
            switch (type) {
                case USER:
                    townData.setUserAlertCnt(count); break;
                case FIRE:
                    townData.setFireAlertCnt(count); break;
                case MOTION:
                    townData.setMotionAlertCnt(count);break;
            }
        }
        townData.setAlertCnt(totalAlertCnt);

        // 4. 마을 관리자 조회
        Long townAdminCnt = em.createQuery("select count(ta.admin) from TownAdmin ta where ta.town.id=:townId", Long.class)
                .setParameter("townId", townId)
                .getSingleResult();
    ...
    public void save(TownData townData) {
