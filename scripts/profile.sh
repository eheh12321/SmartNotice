...
function find_idle_profile() {
    ...
    RESPONSE_CODE=$(curl --max-time 5 -s -o /dev/null -w "%{http_code}" https://smarttownnotice.gq/profile)
    ...
    then
      ...
      CURRENT_PROFILE=$(curl -s https://smarttownnotice.gq/profile) # 사이트에서 현재 사용중인 포트를 응답해줌(real1/real2)
    fi
